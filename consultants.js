...
// Primary Waitlist Form (Chapter 2)
const primaryWaitlistForm = document.getElementById('primary-waitlist-form');
const primarySubmitBtn = document.getElementById('primary-submit-btn');

if (primaryWaitlistForm && primarySubmitBtn) {
    primaryWaitlistForm.addEventListener('submit', async (e) => {
        e.preventDefault();

        const formData = new FormData(primaryWaitlistForm);
        const data = {
            firstName: formData.get('firstName'),
            email: formData.get('email'),
            experience: formData.get('experience'),
            timestamp: new Date().toISOString(),
            source: 'consultant-program-primary-waitlist'
        };

        // Update button state
        const originalHTML = primarySubmitBtn.innerHTML;
        primarySubmitBtn.classList.add('loading');
        primarySubmitBtn.disabled = true;
        primarySubmitBtn.innerHTML = '<span class="button-text">Joining...</span>';

        try {
            // Simulate API call (replace with actual endpoint)
            await new Promise(resolve => setTimeout(resolve, 2000));
            
            console.log('Primary Waitlist Data:', data);
            ...
            // Reset form
            primaryWaitlistForm.reset();
            
        } catch (error) {
            console.error('Primary Waitlist Error:', error);
            alert('Something went wrong. Please try again.');
        } finally {
            primarySubmitBtn.classList.remove('loading');
            primarySubmitBtn.disabled = false;
            primarySubmitBtn.innerHTML = originalHTML;
        }
    });
}